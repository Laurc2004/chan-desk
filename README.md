...
- 🧠 缠论引擎 TypeScript 实现：包含处理 → 分型 → 笔 → 线段 → 中枢 → 10 类买卖点（三买/三卖/中枢突破/一买/一卖/二买/二卖/类二买/类二卖），MACD 背驰自动标注（离开段 vs 进入段 DIF 峰值对比）；二买/二卖带 chan.py 式 retrace_rate 回抽比强度量化（≤0.618 强）
- 📊 决策压力测试：任一信号在 48 个 rToken 标的历史上的全量触发回放，12 根 K 线窗口的胜率/中位收益/盈亏比/最差单笔
...
- 💬 自然语言投研对话（Qwen，Markdown 流式输出，带本地规则回退）
- ⚖️ 人机边界：AI 只给分析，「采纳/忽略」由交易员点击并写理由，决策落档 Supabase 可回看
- 🔄 每 60s 自动增量拉取 Bitget 最新 K 线重算，信号与统计实时滚动

## 实测结论（15m，48 标的，最深至 2025-08，共 1609 信号）
...
结论：确认类买卖点（一二三 + 类二）胜率 70-100%，追中枢突破 19-24%——确认机制而非方向才是 rToken 短线胜负的分水岭。chan.py 的「类二买/类二卖」（bsp2s，三买/三卖确认后的连续回踩）是胜率与样本量兼优的甜点区（合计 536 样本、胜率 70-80%），比一买/二买（样本稀少）更具实盘可操作性。全部实测，未计手续费。

## 技术栈与部署

Next.js 16（App Router）+ TypeScript + Tailwind + klinecharts。缠论引擎与回放统计为纯前端计算，K 线数据为构建期生成的静态 JSON + 浏览器侧每 60s 增量拉取 Bitget 公开 REST（无需 API Key）。部署：Vercel（免费层）；决策档案：Supabase。
...
public/data/   48 标的 × {15m,1H} 静态K线 + 全市场汇总 JSON
```
